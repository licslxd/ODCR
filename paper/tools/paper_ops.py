...
from odcr_core.aux.evidence.ai_analysis_writer import get_writer  # noqa: E402
...
def update_handoff_build_section(
    ...
    get_writer(REPO_ROOT).paper(
        "paper_chat_handoff.md",
        text,
        source="paper_ops.py",
        inputs={"main_tex": str(MAIN_TEX.relative_to(REPO_ROOT))},
        outputs={"pdf_exists": MAIN_PDF.exists(), "build_exit_code": code},
        validation_result={"build_tool": tool, "build_exit_code": code},
    )
...
def handoff() -> int:
    ...
    get_writer(REPO_ROOT).paper(
        ...
        "\n".join(lines).rstrip() + "\n",
        source="paper_ops.py",
        inputs={"decision_path": str(DECISION_PATH.relative_to(REPO_ROOT))},
        outputs={
            "paper_file_count": len(status["paper_files"]),
            "pdf_exists": status["pdf_exists"],
            "missing_cite_keys": len(status["missing_cite_keys"]),
        },
        validation_result="handoff refreshed",
    )
