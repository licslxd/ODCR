...
用户要求继续执行训练、eval、rerank、reclosure、task loop 或类似长运行队列时：

- 默认继续执行到用户请求的任务范围完成，遇到问题先定位并修复，再从失败点续跑。
- 如果用户明确排除某些任务，例如“7/8 不跑”，后续所有 loop、续跑和恢复命令都必须继续排除这些任务。
- 如果用户明确要求“不要频繁播报”，只在失败、阶段切换、需要用户动作、或较长时间间隔的关键进度时简短更新。
- 如果用户明确要求“不写报告”，不要额外生成长篇 final report / phase summary / machine verdict；只保留运行必需的 stdout、status、pid、run meta、现有 ledger 或最小审计记录。
- 长运行期间不得为了写报告、整理说明或重复 probe 打断已经在 GPU pane 中运行的正式任务。
- 如果 fresh rerun 产物被 stage truth / quality gate 拒绝，而 AGENTS 或 stage_status 明确指定了当前任务的已验收历史真值源，
  不要把失败 fresh run 硬推进下游；应显式指定该已验收真值源继续后续阶段，并在最终简短说明中标明失败 run 被跳过。
- 最终交付只汇总完成状态、失败处理、关键 run_id、日志路径和验证结果；不要复述完整日志。
