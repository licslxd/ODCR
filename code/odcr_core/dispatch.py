...
# torchrun 在 code/ 目录下加载的 runner 入口（Step5 generator is retired; RACER-C1 does not use torchrun here）
TORCHRUN_STEP3_SCRIPT = "executors/step3_entry.py"
TORCHRUN_STEP4_SCRIPT = "executors/step4_entry.py"
TORCHRUN_STEP5_SCRIPT = "__retired_step5_generator__"
...
def print_dispatch_routing(command: str) -> None:
    ...
    if command == "step3":
        ...
        print("[Dispatch] step5 -> retired; use racer-c1", flush=True)
    elif command == "eval":
        print("[Dispatch] eval -> retired old Step5 generator eval", flush=True)
    elif command == "eval-matrix":
        print("[Dispatch] eval-matrix -> 多次 step5 runner eval (torchrun)", flush=True)
    elif command == "eval-rerank":
        print("[Dispatch] eval-rerank -> retired old Step5 rerank", flush=True)
        ...
        print(
            "[Dispatch] pipeline -> step3 runner -> step4 runner -> RACER-C1 retrieval",
            flush=True,
        )
