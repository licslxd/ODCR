...
def _find_repo_root() -> Path:
    for candidate in Path(__file__).resolve().parents:
        if (candidate / "configs" / "odcr.yaml").is_file() and (candidate / "odcr").is_file():
            return candidate
...
from odcr_core.aux.runtime.stage_dispatch import classify_repo_command as classify_stage_repo_command
from odcr_core.aux.runtime.stage_dispatch import stage_dispatch_admission_to_dict
...
class BridgeOptions:
    ...
    performance_probe_type: str = "epoch-boundary-memory"
    performance_warmup_steps: int = 5
    performance_measured_steps: int = 20
    performance_max_seconds: int | None = None
...
class BridgeCommandPolicy:
    ...
    def classify_repo_command(
        ...
        *,
        ...
        admission = classify_stage_repo_command(
            argv,
            output_dir=output_dir,
            user_confirmed_formal=user_confirmed_formal,
        )
        payload = stage_dispatch_admission_to_dict(admission)
        allowed = bool(payload.pop("allowed"))
        reason = str(payload.pop("reason"))
        return BridgeCommandClassification(allowed=allowed, reason=reason, **payload)
...
def _ensure_analysis_dirs() -> None:
    ensure_ai_analysis_tree(AI_ANALYSIS)
...
class TmuxGpuBridge:
    ...
    def run_mode(self, options: BridgeOptions) -> dict[str, Any]:
        ...
        if options.dry_run or options.no_send:
            dry_run_success = bool(options.dry_run) or options.mode != "step3-performance-probe"
            ...
            if options.mode == "step3-performance-probe" and not dry_run_success:
...
def runtime_command_argv(options: BridgeOptions, run_id: str) -> tuple[str, ...]:
    ...
    if mode in {"repo-script", "repo-module", "command-file"}:
        raise BridgeError(
            f"{mode} is a retired bridge entrypoint; use a registered repo-command via ./odcr runtime",
            stop_reason="forbidden_mode",
            details={"mode": mode, "allowed_surface": "./odcr runtime"},
        )
    if mode == "repo-script":
...
def build_step3_performance_probe_script(
    ...
    *,
    ...
    probe_type: str = "epoch-boundary-memory",
    candidate_name: str | None = None,
    warmup_steps: int = 5,
    ...
    max_seconds = int(max_seconds_override) if max_seconds_override is not None else max(20, min(int(timeouts.hard_timeout_s) - 5, int(timeouts.hard_timeout_s or 180)))
    max_seconds = max(20, min(max_seconds, int(timeouts.hard_timeout_s or max_seconds)))
    ...
    return (
        ...
        + f"--warmup-steps {int(warmup_steps)} --measured-steps {int(measured_steps)} --max-seconds {max_seconds} "
...
def build_script(
    ...
    if options.mode == "step3-performance-probe":
        return build_step3_performance_probe_script(
            ...
            warmup_steps=options.performance_warmup_steps,
            measured_steps=options.performance_measured_steps,
            max_seconds_override=options.performance_max_seconds,
        )
...
def _add_common_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(  # internal-only bridge sentinel, not One-Control config
        "--timeout",
        default="auto",
        help="auto or explicit seconds; capped by mode safety limits.",
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    step3_startup.add_argument("--task-id", type=int, default=2)  # internal-only validation constant
    step3_startup.add_argument(
        "--validation-slug",
        default="step3_tmux_gpu_bridge_startup_validation_closeout",
        # internal-only validation namespace constant
    )

    step3_perf = sub.add_parser("step3-performance-probe")
    _add_common_args(step3_perf)
    step3_perf.add_argument("--task-id", type=int, default=2)  # internal-only validation constant
    ...
    step3_perf.add_argument(
        ...
        default="epoch-boundary-memory",
        # internal-only bounded probe type constant
    )
    step3_perf.add_argument("--warmup-steps", type=int, default=None, help=argparse.SUPPRESS)
    step3_perf.add_argument("--measured-steps", type=int, default=None, help=argparse.SUPPRESS)
    step3_perf.add_argument("--max-seconds", type=int, default=None, help=argparse.SUPPRESS)
    ...
    micro.add_argument("--kind", choices=("bge-single-batch",), default="bge-single-batch")  # internal-only benchmark constant
    micro.add_argument("--embed-batch-size", type=int, default=512)  # internal-only benchmark constant
    micro.add_argument("--read-chunk-rows", type=int, default=100_000)  # internal-only benchmark constant
    micro.add_argument("--group-shard-size", type=int, default=4_096)  # internal-only benchmark constant
    micro.add_argument("--workers", type=int, default=2)  # internal-only benchmark constant
    micro.add_argument("--bf16", action="store_true", default=True, dest="bf16_enabled")  # internal-only benchmark constant
    micro.add_argument("--no-bf16", action="store_false", dest="bf16_enabled")
    micro.add_argument("--tf32", action="store_true", default=True, dest="tf32_enabled")  # internal-only benchmark constant
    micro.add_argument("--no-tf32", action="store_false", dest="tf32_enabled")
    micro.add_argument(  # internal-only benchmark constant
        "--grouped-text-cache",
        action="store_true",
        default=True,
    ...
    collect.add_argument("--timeout", default="auto", help=argparse.SUPPRESS)  # internal-only collect sentinel
...
def options_from_args(args: argparse.Namespace) -> BridgeOptions:
    ...
    return BridgeOptions(
        ...
        performance_probe_type=getattr(args, "probe_type", "epoch-boundary-memory"),
        performance_warmup_steps=int(getattr(args, "warmup_steps", 5) or 5),
        performance_measured_steps=int(getattr(args, "measured_steps", 20) or 20),
        performance_max_seconds=getattr(args, "max_seconds", None),
