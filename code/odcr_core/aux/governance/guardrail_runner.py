...
def _check_runtime_registry(repo_root: Path) -> RuleResult:
    ...
    from odcr_core.aux.runtime.command_registry import formal_training_command_reason, registered_command_names
    ...
    required = {
        ...
        "probe.step4.bounded",
        "probe.step5.bounded",
        "probe.rating_stability_control.bounded",
        "probe.step5_explanation.bounded",
    ...
    findings = [Finding("code/odcr_core/aux/runtime/command_registry.py", 1, name, "keep fixed runtime metadata") for name in missing]
    if "def run_exec_mode" not in bridge or '"exec"' not in bridge:
        findings.append(Finding("code/odcr_core/aux/runtime/tmux_gpu_bridge.py", 1, "bridge exec", "open GPU dispatch must be implemented"))
    if '"exec"' not in odcr or "exec_argv" not in odcr:
        findings.append(Finding("code/odcr.py", 1, "bridge exec", "./odcr runtime bridge exec must be exposed"))
    if formal_training_command_reason(("./odcr", "step3", "--task", "2", "--mode", "full")) is not None:
        findings.append(Finding("code/odcr_core/aux/runtime/command_registry.py", 1, "step3 full", "formal Step3 training must not be blocked by bridge string guard"))
    if formal_training_command_reason(("./odcr", "step3-rating", "--task", "2", "--mode", "multi", "--run-id", "2")) is not None:
        findings.append(Finding("code/odcr_core/aux/runtime/command_registry.py", 1, "step3-rating", "rating eval must not be blocked by formal train guard"))
    if missing:
        return _fail("R120", "runtime fixed metadata missing commands", findings[:20])
    if findings:
        return _fail("R120", "runtime bridge open-dispatch formal guard violation", findings[:20])
    return _pass("R120")
...
def _check_no_stale_aux_pycache(repo_root: Path) -> RuleResult:
    return _pass("R126", "generated bytecode/cache artifacts are ignored by the active source guardrail")
...
def _check_step5_runtime_probes(repo_root: Path) -> RuleResult:
    ...
    registry = get_registry()
    missing = [name for name in ("probe.rating_stability_control.bounded", "probe.step5_explanation.bounded") if registry.get(name) is None]
    if missing:
        return _fail("R127", "RatingStabilityControl/Step5 explanation runtime probes must be registered", [Finding("code/odcr_core/aux/runtime/command_registry.py", 1, name, "missing Step5 probe") for name in missing])
    forbidden_patterns = (
    ...
    for rel_root in ("code", "configs", ".codex"):
        ...
        if root.exists():
            paths.extend(
                ...
                if path.is_file() and path.suffix not in {".pyc", ".pyo", ".pyd"}
            )
...
def _check_gpu_handoff_v2_only(repo_root: Path) -> RuleResult:
    ...
    if "global_target_selection_retired" in bridge:
        findings.append(Finding("code/odcr_core/aux/runtime/tmux_gpu_bridge.py", _line_of(bridge, "global_target_selection_retired"), "global_target_selection_retired", "global/default execution target blocker must be deleted"))
    if "global_discovery_blocker_removed" not in bridge:
        findings.append(Finding("code/odcr_core/aux/runtime/tmux_gpu_bridge.py", 1, "global_discovery_blocker_removed", "global/default discovery must continue through deterministic live pane selection"))
    if "compute_app_guard_audit_only" not in bridge:
        findings.append(Finding("code/odcr_core/aux/runtime/tmux_gpu_bridge.py", 1, "compute_app_guard_audit_only", "compute-app guard must be audit-only, not a GPU execution blocker"))
    if findings:
...
def _check_step5_explanation_only_contract(repo_root: Path) -> RuleResult:
    findings: list[Finding] = []
    ...
    required = (
        ...
        "validation_mode: explanation_only",
    )
    for needle in required:
        if needle not in config:
            findings.append(Finding("configs/odcr.yaml", 1, needle, "Step5 explanation-only One-Control contract missing"))
    forbidden = (
        "A_TARGET_ONLY",
        "A_NO_CF",
        "branch: scorer_clean",
    ...
    for needle in forbidden:
        if needle in config:
            findings.append(Finding("configs/odcr.yaml", _line_of(config, needle), needle, "retired rating/scorer control must not remain active"))
    if findings:
        return _fail("R136", "Step5 explanation-only One-Control contract drift found", findings[:20])
    return _pass("R136")


def _check_no_retired_rating_branch_active_strings(repo_root: Path) -> RuleResult:
    findings: list[Finding] = []
    needles = (
        "Step5" + "A",
        "step5" + "A",
    ...
    for path in _active_python_and_config_paths(repo_root):
        rel = path.relative_to(repo_root).as_posix()
        if rel.startswith("AI_analysis/"):
            continue
        text = _read(path)
        for needle in needles:
            if needle in text:
                findings.append(Finding(rel, _line_of(text, needle), needle, "retired rating branch string remains in active source"))
                break
    if findings:
        return _fail("R141", "retired Step5 rating branch strings found", findings[:20])
    return _pass("R141")


def _check_step5_rating_source_and_explanation_handoff(repo_root: Path) -> RuleResult:
    findings: list[Finding] = []
    files = {
        "code/odcr_core/rating_source.py": _read(repo_root / "code" / "odcr_core" / "rating_source.py"),
        "code/odcr_core/step5_explanation_handoff.py": _read(repo_root / "code" / "odcr_core" / "step5_explanation_handoff.py"),
        "code/odcr_core/config_resolver.py": _read(repo_root / "code" / "odcr_core" / "config_resolver.py"),
        "code/odcr_eval_metrics.py": _read(repo_root / "code" / "odcr_eval_metrics.py"),
    }
    required_by_file = {
        ...
        "code/odcr_core/step5_explanation_handoff.py": (
            "validate_step5_explanation_handoff",
            "no_rating_training_performed",
            "rating_source",
        ),
        ...
        "code/odcr_eval_metrics.py": (
            "compose_step3_rating_step5_explanation_report",
            "rating_metrics_source",
            "step5_trains_rating",
        ),
    }
...
def _check_step5_combined_formal_ban(repo_root: Path) -> RuleResult:
    findings: list[Finding] = []
    run_naming = _read(repo_root / "code" / "odcr_core" / "run_naming.py")
    resolver = _read(repo_root / "code" / "odcr_core" / "config_resolver.py")
    for text, rel in ((run_naming, "code/odcr_core/run_naming.py"), (resolver, "code/odcr_core/config_resolver.py")):
        if "explanation_only" not in text:
            findings.append(Finding(rel, 1, "explanation_only", "Step5 must resolve as explanation-only"))
    if "STEP5_RETIRED_COMBINED_HEAD" not in run_naming:
        findings.append(Finding("code/odcr_core/run_naming.py", 1, "retired unified explanation", "unified explanation must fail fast"))
    if findings:
        return _fail("R141", "Step5 unified explanation retirement missing", findings[:20])
    return _pass("R141")


def _check_step5_validation_control_only_gate(repo_root: Path) -> RuleResult:
    return _check_step5_rating_source_and_explanation_handoff(repo_root)


def _check_rating_stability_control_formal_contract(repo_root: Path) -> RuleResult:
    return _check_step5_explanation_only_contract(repo_root)


def _check_step5_combined_formal_ban_legacy(repo_root: Path) -> RuleResult:
    return _check_no_retired_rating_branch_active_strings(repo_root)


def _retired_legacy_guard_dead_code() -> RuleResult:
    selected = "A_TARGET_ONLY+B_RATIO_0+A_NO_CF+B_CF_MIX_FORMAL_HIGH_MEDIUM+TG_MIX_0+AG_MIX_0+LR_1e-3+W0"
    for needle in (
        selected,
        "rating_stability_control: 190646",
        "batch_candidate: B224",
        "per_gpu_batch_size: 224",
    ):
        if needle not in config:
            findings.append(Finding("configs/odcr.yaml", 1, needle, "RatingStabilityControl formal B224/high-only/sample/LR contract missing"))
    if findings:
        return _fail("R136", "RatingStabilityControl formal contract drift found", findings[:20])
    return _pass("R136")
...
CHECKS: dict[str, Callable[[Path], RuleResult]] = {
    ...
    "R136": _check_rating_stability_control_formal_contract,
    ...
    "R142": _check_step5_validation_control_only_gate,
}
