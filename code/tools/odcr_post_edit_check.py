        ...
        CheckCommand(
            ...
            argv=(
                ...
                (
                    ...
                    "import odcr_core.config_resolver as cr; "
                    "from odcr_core.config_resolver import resolve_config; "
                    "from test_step5_graph_safety import _write_step4_upstream_fixture; "
                    ...
                    "eval_profile='balanced_2gpu', mode='full')\n"
                    "finally:\n"
                    "    cr._REPO_ROOT=old\n"
                    "    tmp.cleanup()\n"
                ),
        ...
        try:
            ...
            if proc.returncode == 0:
                ...
                if classification == "resource_kill" and _is_single_test_command(command):
                    print(f"[{idx}/{len(commands)}] RERUN {command.label} after resource kill", flush=True)
                    rerun_started = time.monotonic()
                    try:
                        rerun_proc = subprocess.run(command.argv, cwd=repo_root, env=env, timeout=max_seconds)
                        rerun_elapsed = time.monotonic() - rerun_started
                        rerun_returncode = int(rerun_proc.returncode)
                        rerun_status = "PASS" if rerun_proc.returncode == 0 else "FAIL"
                        final_classification = "flaky_resource_kill" if rerun_proc.returncode == 0 else "semantic_fail"
                    except subprocess.TimeoutExpired:
                        rerun_elapsed = time.monotonic() - rerun_started
                        rerun_status = "TIMEOUT"
                        final_classification = "timeout"
                status = "PASS" if final_classification == "flaky_resource_kill" else "FAIL"
