...
class TestStep3FailedLatestPointerRejection(unittest.TestCase):
    def test_failed_latest_and_failure_audit_block_downstream(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            run_id = "11"
            meta = repo / "runs" / "step3" / "task2" / run_id / "meta"
            meta.mkdir(parents=True)
            summary = meta / "run_summary.json"
            summary.write_text(json.dumps({"run_id": run_id, "status": "failed"}), encoding="utf-8")
